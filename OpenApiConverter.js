"use strict";
/** Converter for transforming OpenAPI specification JSON into 'CodeAst' structures
 * @author TeamworkGuy2
 */
...
(function (OpenApiConverter) {
    ...
    /** Extran CodeAst types from Open API JSON models
     * @param openApiDefinitions a map of Open API JSON model names to model definitions to extract
     * @param typeConverter converter for field types, i.e. convert an Open API type like 'number' or 'integer' with an optional format like 'float' or 'int64' to a 'CodeAst.Type'
     * @param [modelNamer] optional, function to create a model name based on the location and name of the model being extracted
     * @see readType()
     */
    function extractOpenApiModels(openApiDefinitions, typeConverter, modelNamer) {
    ...
    function createModelParameterFromType(paramName, paramType, openApiDefinitions, typeConverter, modelNamer) {
        if (modelNamer === void 0) { modelNamer = OpenApiConverter.defaultModelNamer; }
        var models = {};
     ...
     * 'OpenApiConverter.typeToString' must be set to read 'anyOf' and 'allOf' types
     ...
     * @param openApiDefinitions nullable, the OpenAPI document root 'definitions' for looking up '$ref' types, reading '$ref' types will fail if this is null
    ...
    function readType(nameStack, openApiType, objectVisitor, openApiDefinitions, modelNamer, typeConverter) {
        ...
        // 'allOf' schema
            ...
            objType.classSignature.annotations = addAll(objType.classSignature.annotations, [{ name: "AllOf", arguments: { value: JSON.stringify(allTypes.map(function (type) { return OpenApiConverter.typeToString(type); })).replace(/\",\"/g, "\", \"") } }]);
        ...
        // 'anyOf' schema
            ...
            objType.classSignature.annotations = addAll(objType.classSignature.annotations, [{ name: "AnyOf", arguments: { value: JSON.stringify(anyTypes.map(function (type) { return OpenApiConverter.typeToString(type); })).replace(/\",\"/g, "\", \"") } }]);
            objectVisitor(objType);
            return {
                name: last(nameStack),
                type: typeConverter(objType.classSignature.name),
